...
<!--    <div v-else>
      <slot>
      <div class="calendly-inline-widget" data-url="https://calendly.com/caeappt/speakwithanexpert" style="min-width:320px;height:700px;"></div>
      </slot>
    </div>-->
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    emitAnswer(q) {
      if (q) {
        if (!q.focused) {
         ...
         // console.log("answer", this.calendarCondition.quizId)
          //console.log("quist,", quiz.question.index)

          if (quiz.question.id === this.calendarCondition.quizId && quiz.question.answer === this.calendarCondition.quizAnswer && quiz.question.answered) {

            this.showCalendar = true;

           // console.log("k-------this.showCalendar------>>>>>>>", this.showCalendar)

           const stepData = {
            currentStepIndex: quiz.question.index,
            showCallendar:  this.showCalendar = true,
           }

            this.$emit('calendar', stepData )
    ...
    showOkButton() {
      ...
      console.log("mmmmmmmmmm", this.question)
