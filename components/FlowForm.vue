    ...
    <div class="f-container">
      <div class="f-form-wrap">
        ...
        <flow-form-question :ref="setQuestionRef" v-for="(q, index) in questionList" v-bind:question="q"
          ...
          v-on:disable="setDisabled" v-bind:autofocus="autofocus" v-bind:calendarCondition="calendarCondition"
          v-on:calendar="onShowCalendar" />
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      disabled: false,
      showCalendar: false,
      calendarCondition: this.launchCalendarCondition,
    }
  },

  mounted() {

    console.log("mounted calen",this.showCalendar)
    if(this.nextStepIndex){
      console.log("mounted and calendar", )
      this.goToQuestion(this.nextStepIndex)
  ...
  methods: {
    ...
    isNextQuestionAvailable() {
      console.log("is there another question to jump to?", this.launchCalendarCondition.quizAnswer)
      if (this.submitted) {
        return false
      }
      ...
      //console.log(isProxy(q) ? 'yup' : 'nope')
      const quiz = Object.assign({}, q);

           //console.log("k-------question indes------>>>>>>>",quiz.index )

      if (quiz.id === this.launchCalendarCondition.quizId && quiz.answer === this.launchCalendarCondition.quizAnswer) {

        // this.showCalendar = true;

        //console.log("k------------->>>>>>>", this.showCalendar)
      }
    ...
    onQuestionAnswered(question) {
      ...
      if (question.isValid()) {
        ...
        if (this.activeQuestionIndex < this.questionListActivePath.length) {
          ...
          //console.log("active quiz index", this.activeQuestionIndex)
        }
    ...
    onShowCalendar(value) {
      //console.log("----------on show ----calendar", value)

      this.$emit('calendar', value)
    },
