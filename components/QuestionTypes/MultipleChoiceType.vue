<template>
  <div class="f-radios-wrap">
    <ul class="f-radios" v-bind:class="{ 'f-multiple': question.multiple }" role="listbox">
      ...
      <li v-for="(option, index) in question.options" v-on:click.prevent="toggleAnswer(option)"
        ...
        <span class="f-image" v-if="hasImages && option.imageSrc">
          ...
          <!--   <img :src="`~/assets/images/${option.value}.jpg`" :alt="option.imageAlt">-->

          <img v-if="option.value === 'Sedan'" src="~/assets/images/Depositphotos_7486411_S.jpg" alt="icon">
          <img v-if="option.value === 'SUV'" src="~/assets/images/Depositphotos_526166136_S.jpg" alt="icon">
          <img v-if="option.value === 'Truck'" src="~/assets/images/Depositphotos_564946972_S.jpg" alt="icon">
          <img v-if="option.value === 'Van'" src="~/assets/images/Depositphotos_353686092_S.jpg" alt="icon">
      ...
      <li class="f-other" v-if="!hasImages && question.allowOther" v-on:click.prevent="startEditOther"
        v-bind:class="{ 'f-selected': question.other, 'f-focus': editingOther }" v-bind:aria-label="language.ariaTypeAnswer"
        role="option">
        <div class="f-label-wrap">
          <span class="f-key" v-if="!editingOther">{{ getToggleKey(question.options.length) }}</span>
          <input v-if="editingOther" v-model="question.other" type="text" ref="otherInput" v-on:blur="stopEditOther"
            v-on:keyup.enter.prevent="stopEditOther" v-on:keyup="onChangeOther" v-on:change="onChangeOther"
            maxlength="256" />
